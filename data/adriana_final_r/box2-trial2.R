...
#trim_data<-subset(Data,hours>=0.5 &Data,hours<=8) #After 30 min #SYNTAX ERROR - this included all data!
trim_data<-subset(Data, hours>=0.5 & hours<=8) #CORRECTED
...
#plot(x=eTime[DatQ==TRUE],y=RMR_micr[DatQ==TRUE],ylim=c(16,18),xlab = "eTime (minutes)",ylab = O2_Units, main= "Control")
#plot(x=eTime[DatQ==TRUE],y=RMR_02[DatQ==TRUE],ylim=c(15,18),xlab = "eTime (minutes)",ylab = O2_Units, main= "B2C2")
#plot(x=eTime[DatQ==TRUE],y=RMR_03[DatQ==TRUE],ylim=c(15,19),xlab = "eTime (minutes)",ylab = O2_Units, main= "B2C3")
#plot(x=eTime[DatQ==TRUE],y=RMR_04[DatQ==TRUE],ylim=c(15,18),xlab = "eTime (minutes)",ylab = O2_Units, main= "B2C4")

#Calc MO2 for range
bad_data = trim_data$hours >= 1
