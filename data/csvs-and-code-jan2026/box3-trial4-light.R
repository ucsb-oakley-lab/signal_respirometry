...
#Data<-read.csv("box3-trial4-light.csv")
Data<-read.csv("/Users/oakley/Documents/GitHub/signal_respirometry/data/csvs-and-code-jan2026/box3-trial4-light.csv")
...
RMR_micr<- trim_data$Ch1_kPa
#RMR_02<- trim_data$Ch2_kPa  # Ch2 missing from this trial
RMR_03<- trim_data$Ch3_kPa
RMR_04<- trim_data$Ch4_kPa

DatQ<- rep(TRUE,times=length(eTime))
Temp<- median(trim_data$Temp)
#Temp was hardcoded as column 7, but that's wrong when Ch2 is missing
...
#plot(x=eTime[DatQ==TRUE],y=RMR_03[DatQ==TRUE],ylim=c(10,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "B3C3")
#plot(x=eTime[DatQ==TRUE],y=RMR_04[DatQ==TRUE],ylim=c(4,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "B3C4")
