...
#Data<-read.csv("box3-trial5-light.csv")
Data<-read.csv("/Users/oakley/Documents/GitHub/signal_respirometry/data/csvs-and-code-jan2026/box3-trial5-light.csv")
...
#Establishing the data from the chambers. CSV has: seconds, hours, Ch2, Ch3, Ch4, Temp
...
##plot(x=eTime[DatQ==TRUE],y=RMR_micr[DatQ==TRUE],ylim=c(10,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "Control")
#plot(x=eTime[DatQ==TRUE],y=RMR_02[DatQ==TRUE],ylim=c(5,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "B3C2")
#plot(x=eTime[DatQ==TRUE],y=RMR_03[DatQ==TRUE],ylim=c(5,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "B3C3")
#plot(x=eTime[DatQ==TRUE],y=RMR_04[DatQ==TRUE],ylim=c(3,20),xlab = "eTime (minutes)",ylab = O2_Units, main= "B3C4")
...
RMRD02$MO2<-RMRD02$MO2-0.006510285 #Taken from newpyro-trial5-light
RMRD03$MO2<-RMRD03$MO2-0.006510285 #Taken from newpyro-trial5-light
RMRD04$MO2<-RMRD04$MO2-0.006510285 #Taken from newpyro-trial5-light
...
#B3C2
B3C2<-conv_resp_unit(value=RMRD02$MO2_mass_specific, from="umol_O2 / g / hr", to= "ml_O2 / mg / hr", temp = Temp, sal = Sal, atm_pres = 1013.25)
B3C3<-conv_resp_unit(value=RMRD03$MO2_mass_specific, from="umol_O2 / g / hr", to= "ml_O2 / mg / hr", temp = Temp, sal = Sal, atm_pres = 1013.25)
B3C4<-conv_resp_unit(value=RMRD04$MO2_mass_specific, from="umol_O2 / g / hr", to= "ml_O2 / mg / hr", temp = Temp, sal = Sal, atm_pres = 1013.25)
